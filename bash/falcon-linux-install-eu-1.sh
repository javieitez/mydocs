...
#10/07/2025, riikka.rosendahl added fix under patch_tags - sleep 60 to fix issues random error HTTP 404 when adding tags
...
Optional:
    ...
    - FALCON_GROUPING_TAGS              (default: unset) format: "tag1,tag2,tag3" no whitespace allowed
    - FALCON_INSTALL_ONLY               (default: false)
    - FALCON_TAGS_ONLY                  (default: unset)
...
main() {
    ...
    if [ ! -z "${FALCON_GROUPING_TAGS}" ]; then
        echo -n 'Falcon Grouping Tags ... '
        validate_tags "${FALCON_GROUPING_TAGS}"
        patch_tags $(get_aid_local) "${FALCON_GROUPING_TAGS}"
        echo 'Falcon Grouping Tags Applied'
    fi
...
# Get local AID from Falcon sensor
get_aid_local() {
    if ! command -v /opt/CrowdStrike/falconctl &> /dev/null; then
        die "Falcon sensor not found or falconctl not accessible"
    fi
    
    local aid
    for i in $(seq 12); do
        aid=$(/opt/CrowdStrike/falconctl -g --aid 2>/dev/null | cut -f 2 -d = | tr -d '"' | tr -d .)
    
        if [[ -z "${aid}" ]] || [[ "${aid}" == "aid is not set" ]]; then
            sleep 10
        else
            break
        fi
    ...
    if [[ -z "${aid}" ]] || [[ "${aid}" == "aid is not set" ]]; then
        die "Failed to retrieve aid from local Falcon sensor"
    fi
...
patch_tags() {
    ...
    local prefixed_tags=""
    local tags_json=""
    echo "Waiting 60 seconds to apply tags"
    sleep 60

    prefixed_tags=$(add_tag_prefix "${tags}")

    tags_json=$(generate_json_csv ${prefixed_tags})

    tag_response=$(curl_command -S -X PATCH "https://$(cs_cloud)/devices/entities/devices/tags/v1" \
    ...
    if [ ! "${tag_response:0:1}" = "2" ]; then
        die "HTTP ${tag_response} from tag request"
    fi
...
add_tag_prefix() {
    ...
    local result=""
    local first=1
    local tag_prefix="FalconGroupingTags"
    
    for item in ${items}; do
        if [ $first -eq 1 ]; then
            result="${tag_prefix}/${item}"
            first=0
        else
            result="${result},${tag_prefix}/${item}"
        fi
    done
...
generate_json_csv() {
    ...
    local json_tags
    json_tags=$(jq -Rn -r -c --arg tags "${tags}" '$tags | split(",") | map(gsub("^\\s+|\\s+$"; ""))')
    
    if [[ $? -ne 0 || -z "${json_tags}" ]]; then
        die "Failed to convert tags to JSON format"
    fi
...
validate_tags() {
    ...
    for tag in $TAGS; do
        ...
        if ! [[ $tag =~ ^[a-zA-Z0-9_/-]+$ ]]; then
            die "Tag '$tag' contains invalid characters. Only alphanumeric characters, hyphens, and underscores are allowed."
        fi
    done
